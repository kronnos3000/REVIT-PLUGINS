...
$Iss       = Join-Path $PSScriptRoot "CCorpPrint.iss"
...
if (-not $Version) {
    $csproj = Join-Path $RepoRoot "CCorpPrint\CCorpPrint.csproj"
...
$setupExe = Join-Path $DistRoot "installer\CCorpPrint-Setup-$Version.exe"
...
if (Test-Path $pfx) {
    ...
    if ($signtool) {
        ...
        & $signtool sign /f $pfx /p "changeme" /fd SHA256 /tr "http://timestamp.digicert.com" /td SHA256 /d "CCorpPrint" $setupExe
