# Deploy.ps1 — Build and install CCorpPrint to the chosen Revit year's addins folder.
#
...
Write-Host "Building CCorpPrint for Revit $Year ($Config)..." -ForegroundColor Cyan
& $Msbuild "$ProjectDir\CCorpPrint.csproj" /p:Configuration=$Config /p:Platform=AnyCPU "/p:RevitYear=$Year" /nologo /verbosity:minimal
...
Copy-Item "$OutputDir\CCorpPrint.dll"    $AddinTarget -Force
Copy-Item "$ProjectDir\CCorpPrint.addin" $AddinTarget -Force
